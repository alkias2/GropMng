...
internal sealed class ContainerSeeder
{
    ...
    // (ContainerType, BaseCm, RimCm, HeightCm, VolumeL)
    ...
    // Returns list of Container IDs ordered by TempId (1–37)
    public async Task<IReadOnlyList<int>> SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            // Material encodes "Πήλινη" / "Πλαστική" / "Παρτέρι"
            var container = new Container
            {
                OwnerId = DemoOwnerBusinessId,
                ContainerType = c.ContainerType,
                Material = c.MaterialLabel,
                DiameterCm = c.RimCm.HasValue ? c.RimCm / (decimal)Math.PI : null, // rim circumference → approx. diam
                DepthCm = c.HeightCm,
                // Beds (ground) have no real volume; store 0 so the DB dimension constraint is satisfied
                ...
                Notes = c.RimCm.HasValue ? $"Βάση: {c.BaseCm} cm | Χείλος: {c.RimCm} cm" : null,
                CreatedAtUtc = now,
                UpdatedAtUtc = now
            };
